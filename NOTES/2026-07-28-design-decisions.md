...
### QD-05 "Meet the parabola" built — the unit's third face, and the Parabola Lab engine
- The graphical spine. Extended the Intermediate Straight Lines coordinate engine into a reusable **Parabola Lab** (`drawParab(svgId, opts)`): fixed/auto y-window, gridlines + axes + italic x/y labels reused from the line engine, plus new curve sampling (y = a(x−p)²+k → SVG polyline), a dashed line-of-symmetry overlay, a ringed turning-point marker, plotted-point list, and a highlight point with drop-lines. Built once; will be preset for QD-06 (a-slider), QD-07 (p-slider), QD-08 (real parabolas). Confirms the blueprint's "extend, don't rewrite" resource-reuse plan.
- Interactive (Gerry chose **Table → Plot builder** from a 4-option bank). Connection (primary): fill y=x² for x=−3..3, Check & plot drops the correct points, Join into a curve reveals the U + symmetry line + turning point; mirror pairs named in the readout. Movement: predict-the-height (substitute & square, point lands on curve; negative-x feedback names "squaring the negative made it positive"). Reflection: The mirror — find a point's twin across x=0. Creativity (stretch, folded in from the "between the dots" option): plot a half-way x (1.5/2.5) to show the curve is continuous. Rest + Nutrition Mode B (symmetry as certainty/economy). Reconnect: substitute (x²@4=16), squaring a negative ((−5)²=25), plot a coordinate. 6 practice Qs. Palette n→x switched to ["x","²","(",")","−","+","="]. qa_quad05.js 24 checks → **32/32**.
- The conceptual core is that **squaring a negative gives a positive**, so the two arms rise together and the curve is symmetric — this is the emotional/"aha" beat, and the top misconception the Adult guide targets ((−3)²=−9).
- Guides: new reusable **`parab_diagram`** in _guides_template.py (matplotlib: green y=x² curve, plotted points, dashed symmetry line, highlighted mirror points, turning-point ring; italic $x$/$y$ via mathtext cm; point labels use the true minus "−"). `_wex_card` stacked branch now accepts "parabola". Two Learner worked examples (build-from-table; find-the-mirror-twin) verified visually — parabola, symmetry, ring, aligned italic-x working all correct.
- Site builds **46 live / 0 missing**. Curriculum + build_site SOURCES updated (quad-05-meet-the-parabola).
- Open consistency question for Gerry: the interactive italicises both x and y (via .uk class); the guides italicise only x (the fix()/mathx pipeline). Deferred a decision on whether to italicise standalone y across the whole guide corpus.

### QD-06 "Stretch and reflect (y = ax²)" built — first transformation lesson
- Gerry's steer: build the **a-dial** (live slider) as the Movement hero (his Option 1), and fold **table compare** (Option 2) in elsewhere → it became the Connection panel. Flow: build it (table) → feel it (dial) → read it back (name the a) → design it (through a point).
- Parabola Lab extended (not rewritten): added **ghost reference curve** (`opts.ghost {a,p,k}` faint dashed, with `ghostLabel`) and **multi-curve** support (`opts.curves=[{a,col,dash}]`) so three parabolas render on one grid. Same engine, two small additions — continues the reuse plan.
- Panels: Connection "Same curve, scaled" (fill y=2x² and y=½x², plot all three vs grey-dashed y=x²; legend). Movement (primary) "The a-dial" (slider −3..3 step 0.5; ghost y=x² kept still; readout names narrower/​wider/​reflected; a=0 gives a flat-line note; turning point ring stays at (0,0)). Reflection "Name the a" (secret-a curve + ghost; catches the 'bigger = wider' slip). Creativity (stretch) "Make it pass through a point" (a = y ÷ x²; e.g. (2,8)→2, (2,2)→0.5, (1,−3)→−3). Rest + Nutrition Mode B (transformation = one control governing a family). Palette adds "a". Reconnect: multiply a height (2×9=18), half of a square (½×16=8), negative coefficient (−1×2²=−4). 6 practice. qa_quad06.js **29/29**.
- Core misconception targeted throughout: **bigger a → narrower, not wider** (heights are multiplied, so they climb faster). Also 2x² ≠ (2x)² (square first, then multiply). Both in the Adult guide.
- Guides: `parab_diagram` extended with `a` and `ghost` params (draws y=ax² with the y=x² ghost); `_wex_card` passes them. Two Learner worked examples (stretch y=2x² with ghost + doubled-height points; find-a-through-(2,12) with stacked fraction 12/4=3) verified visually — clean italic x, corrected the "y = ax²" step (was mis-rendered as a fraction).
- Site builds **47 live / 0 missing**. build_site SOURCES + curriculum updated (quad-06-stretch-and-reflect). Next: QD-07 "Slide it across (y = (x − p)²)" — reuses the same engine with the p-slider + moving turning point / axis of symmetry.

### QD-06 fix (Gerry): accept fractions in the table-compare ½x² column
- Learners entering "1/2" (or "½") for y = ½x² were marked wrong — the tolerant parser read "1/2" as 1. Added a fraction-aware `numFrac()` (handles a/b, ½, ¼, ¾, decimals) used for the Connection table-compare check, so 1/2, ½ and 0.5 are all accepted; whole numbers unaffected. qa_quad06 now 31/31 (added a numFrac unit check + a 1/2-entry check).

### Root-cause fix for the recurring "correct answer, wrong form" defect (Gerry)
- Gerry flagged that the 1/2-vs-0.5 rejection is the same class of bug we thought we'd already solved. Investigated: GreenPrint entry v2.0 (point 4) said marking "stays generous" but illustrated only WORD synonyms; it never named equivalent NUMERIC forms, gave no shared implementation, and — crucially — added no QA gate. And the shared num() in the skill's assets/reference-shell.html was Number(clean(v)), where Number("1/2") = NaN. So the principle was prose-only and each lesson re-introduced the gap; only caught by hand.
- Fix at three levels so it can't recur: (1) GreenPrint entry **v2.1** filed (vault/00_GreenPrint_and_Governance/..._v2.1_Entry_Equivalent_Answer_Forms_DRAFT.md) documenting the recurrence, root cause, the fraction-aware num(), and QA gate 6a. (2) The **neo-mathematics skill SKILL.md updated** (save_skill) to carry the fraction-aware num() verbatim as a MUST-use standard, note it supersedes the reference-shell num(), and state QA gate 6a (harness must assert a fraction form is accepted wherever any correct answer is non-integer; a rejected valid fraction is a ship-blocker). (3) This note. NB save_skill only replaces SKILL.md and keeps the other skill files, so qa-gates.md/reference-shell.html in the cache are unchanged — SKILL.md now carries the authoritative override and is read in full before every build.
- Standard num() to reuse in all future lessons:
  clean: strip commas/space, ½→0.5 ¼→0.25 ¾→0.75, unicode − → -; then match a/b fraction, else Number().

### QD-07 "Slide it across (y = (x − p)²)" built — horizontal translation
- Gerry: p-slider as Movement hero + table-shift folded into Connection (same pattern as QD-06). First lesson built on the **new GreenPrint v2.1 answer-parser standard** — top num() is the fraction-aware clean()/num(); qa_quad07 asserts gate 6a (num("1/2")=0.5, num("½")=0.5, num("−4")=-4).
- Parabola Lab extended once more: multi-curve `opts.curves` now takes per-curve p and k (was a-only), so two shifted curves render on one grid for the table-shift.
- Panels: Connection "The same curve, moved sideways" (fill (x−2)², plot beside y=x², see identical heights shifted right, lowest point (0,0)→(2,0)). Movement (primary) "The p-slider" (drag p −3..3, curve + turning point (p,0) + line of symmetry x=p slide together against ghost y=x²; readout surfaces the "minus moves right" reason via bracket=0). Reflection "Find the bottom" (read turning point incl. (x+1)²→x=−1 sign trap). Creativity (stretch) "Write the equation" (choose p so lowest point lands on a target; reverse). Rest + Nutrition Mode B (trust the reason under a surprising rule). Palette adds "p". Reconnect: (5−2)²=9, x−3=0→3, (−3)²=9. 6 practice. qa_quad07 **27/27**.
- Core misconception targeted throughout: **(x − 2)² slides RIGHT, not left** — reliable check is "set the bracket to 0". Also (x+1)² turning at x=−1, and (x−2)²≠x²−4. All in the Adult guide.
- Guides: parab_diagram extended with `p` (draws y=a(x−p)² with y=x² ghost). Two Learner worked examples (slide-right table; write-the-equation for (−2,0)) verified visually.
- Site builds **48 live / 0 missing**. Next: QD-08 "Reading real parabolas" (the applied finale; transformation compare + Mode A where a context genuinely lives in food/growing/budgeting).

### Guide parabola diagrams centred on the vertex for symmetry (Gerry)
- Gerry: in the guide worked-example graphs, a shifted parabola (e.g. (x−2)²) showed a full left arm but only a stub of the right — lopsided, because the window was fixed symmetric about x=0 while the vertex sat at x=p. For neurodivergent learners the parabola should LOOK symmetric, consistently.
- Fix in _guides_template.py parab_diagram: centre the x-window on the curve's line of symmetry x=p with a symmetric radius, and size the height from the curve's own arms (target ~11, R = max(sqrt(11/|a|), furthest point +0.6, 2.6)). So every worked-example parabola shows both arms equally at a consistent scale; the y=x² ghost still shows (its vertex visible) as the reference. Line-of-symmetry label repositioned to the new yhi.
- Regenerated QD-05, QD-06, QD-07 learner + adult guides for a uniform frame. Verified visually: QD-07 (x−2)² now vertex-centred, both arms to ~11, points symmetric about x=2. Lesson HTML/QA untouched (guides only). Site 48 live / 0 missing.

### QD-08 "Reading real parabolas" built — UNIT COMPLETE (8/8)
- Gerry: option 1 (Parabola in the world) as hero + fold in option 2 (max/min) and option 3 (fence, Mode A). Flow: meet → classify → interpret → model.
- Panels: Connection "The shape is everywhere" (thrown ball ∩, read the top). Movement "Maximum or minimum?" (classify real curves up/down; a<0→max, a>0→min). Reflection (HERO) "The parabola in the world" (read a turning point from labelled axes, then reveal its real meaning — ball 9 m at 3 s; revenue £25 at £5; cost £3 at 400). Creativity "The biggest growing space" (20 m fence, area=w(10−w), slider finds max 25 m² at a 5×5 square). Rest + Nutrition **Mode A** honest (food-growing/sustainability; ball/fountain named as settings, kept Mode B). Reconnect: max/min, read a turning point, substitute into a model. 6 practice. qa_quad08 **24/24** (incl. gate-6a parser assertion).
- Parabola Lab: added opts.xLabel/yLabel (context axis labels) + highlight.noLabel (suppress duplicate coord label under the turning-point ring). Downward parabolas (a<0) and general vertex (p,k) already supported.
- Guides: parab_diagram extended with k (vertical shift), xlabel/ylabel, and a turning-point coordinate annotation gated to context diagrams (so QD-05/06/07 rings stay unlabelled). R target 11→14 for a fuller arc on a<0 contexts (negligible for a=1 lessons, so QD-05/06/07 left as committed — no churn). Two Learner worked examples (greatest height; best price for most revenue) verified visually — ∩ arc, ringed labelled turning point, context axes.
- **Quadratics unit now 8/8 lessons live** (QD-01 growing patterns · QD-02 second difference · QD-03 rule-behind-the-sequence bridge · QD-04 expression · QD-05 meet the parabola · QD-06 stretch & reflect · QD-07 slide it across · QD-08 reading real parabolas). Site 49 live / 0 missing. Four faces of the quadratic — number, rule, symbol, shape — connected end to end.
- Follow-up options for Gerry: offer to update the Foundations Coverage Map with the unit's Stepping Stones/Reconnection Routes/evidence; the deferred italic-x sweep across interactive lessons (task #84); the Year-10 nth-term-via-simultaneous-equations lesson (task #91).
